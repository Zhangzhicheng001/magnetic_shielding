...
static void GT911_WR_Reg_One_byte(uint16_t reg,uint8_t data)
{
	...
	temp[1]=reg&0xff;
	temp[2]=data;
	if(HAL_I2C_Master_Transmit(&hi2c2,GT_CMD_WR,temp,3,0xff)!=HAL_OK)
...
static void GT911_RD_Reg(uint16_t reg,uint8_t *buf,uint8_t len)
{
	...
	temp[1]=reg&0xff;
	temp[0]=reg>>8;
	if(HAL_I2C_Master_Transmit(&hi2c2,GT_CMD_WR,temp,2,0xff)!=HAL_OK)
		printf("read reg error\r\n");
	if(HAL_I2C_Master_Receive(&hi2c2,GT_CMD_RD,buf,len,0xff)!=HAL_OK)
		printf("read reg error\r\n");
}
